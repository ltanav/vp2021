<?php
    ...
    require_once("../../config_vp2021.php");
	...
	if (isset ($_POST["person_photo"])){
		//var_dump($_POST);
		var_dump($_FILES);
		$image_check= getimagesize($_FILES ["photo_input"])
		if($image_check !== false){
			if($image_check ["mine"] =="image/jpeg"){
				$file_type= "jpg";
			}
			if($image_check !== false){
			if($image_check ["mine"] =="image/png"){
				$file_type= "png";
			}
			if($image_check !== false){
			if($image_check ["mine"] =="image/gif"){
				$file_type= "gif";
			}
			//teen ajatempli
			$time_stamp= microtime(1) * 1000;
			//moodustan failinime
			$file_name= "person_" .$_POST ("person_select_for_picture").".".
			$time_stamp."." .$file_type;
			if(move_ uploaded_files())
